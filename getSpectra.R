...
plot(rawspec[,1],rawspec[,2],type="h",lwd=1, xlim=c(200, 1300), xlab="mz", ylab="int", main="raw.dia")
### normalize heights
normheights1<-secondpep[,2]/max(secondpep[,2])
normheights2<-f.secondpep[,2]/max(f.secondpep[,2])
plot(secondpep[,1],-normheights1,type="h",col="red",lwd=1, xlim=c(200, 1300),
     ylim=c(-1,1),xlab="mz", ylab="int", main="projected(top) vs. library(bot)")  
lines(f.secondpep[,1],normheights2,type="h",col="black",lwd=1, xlim=c(200, 1300),xlab="mz", ylab="int")  
abline(h=0)
